<!--
  Shared repository instructions. Codex and Cursor read this file directly;
  Claude Code imports it through .claude/CLAUDE.md.
-->
...
Create in order, then wire in the matching process composition file (`internal/app/httpapi/httpapi_*_wiring.go` for HTTP or `internal/app/grpcapi/grpcapi_*_wiring.go` for gRPC):
...
7. For HTTP changes: update `contracts/http/openapi.yaml` first, run `make openapi-generate`, then adapt handlers.
8. For gRPC contract changes: update `contracts/grpc/` first, run `make proto-lint proto-generate`, then adapt services.
9. Use binding tags on DTOs (`trim:"false"`, `case:"lower"`, `case:"upper"`); no manual trim/case.
10. Follow **File and directory naming** conventions above.
11. Avoid redundant passes over the same data unless clarity or separation is worth it.
