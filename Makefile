...
.PHONY: install run run-httpapi run-httpapi-stop run-grpcapi run-grpcapi-stop run-grpcclient fmt-check vet build check test test-cover test-race test-grpc test-integration test-all ci openapi-generate proto-generate proto-lint proto-breaking proto-generated-check sqlc-generate migrate-up migrate-down migrate-status dev-up dev-down dev-logs check-goose-dbstring openapi proto proto-check sqlc mup mdown mstatus
