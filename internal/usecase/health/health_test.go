...
func TestCheckReadySuccess(t *testing.T) {
	...
	kv := &stubHealthStore{}
	uc := New(stubLogger{}, observability.NoopTracer{}, observability.NoopMeter{}, db, cache, kv)
	...
	if db.pingCalls != 1 || db.statusCalls != 1 || cache.calls != 1 || kv.calls != 1 {
		t.Fatalf("unexpected dependency call counts: dbPing=%d dbStatus=%d cache=%d kv=%d", db.pingCalls, db.statusCalls, cache.calls, kv.calls)
	}
}
...
func TestCheckLiveSkipsDependencies(t *testing.T) {
	...
	if db.pingCalls != 0 || db.statusCalls != 0 || cache.calls != 0 || kv.calls != 0 {
		t.Fatalf("dependencies should not be called in live mode")
	}
}

func TestCheckInvalidMode(t *testing.T) {
	uc := New(stubLogger{}, observability.NoopTracer{}, observability.NoopMeter{}, &stubDBHealthRepo{}, &stubHealthStore{}, &stubHealthStore{})
...
func TestNewWithNilTracerDoesNotPanic(t *testing.T) {
	uc := New(stubLogger{}, nil, observability.NoopMeter{}, &stubDBHealthRepo{}, &stubHealthStore{}, &stubHealthStore{})
