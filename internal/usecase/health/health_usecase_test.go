...
func (s *stubDBHealthRepo) GetServerStatus(context.Context) (repodb.DBServerStatus, error) {
	s.statusCalls++
	if s.statusErr != nil {
		return repodb.DBServerStatus{}, s.statusErr
	}
	return s.status, nil
}
...
func TestCheckCacheFailure(t *testing.T) {
	db := &stubDBHealthRepo{
		status: repodb.DBServerStatus{DatabaseName: "app", UptimeSeconds: 10},
	}
