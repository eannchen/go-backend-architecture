...
INSTALL_TOOLS := github.com/air-verse/air@latest $(SQLC_CMD) $(GOOSE_CMD)
INTEGRATION_PACKAGES := \
	./internal/infra/db/postgres/store \
	./internal/infra/cache/redis/store \
	./internal/infra/kvstore/redis/store

.PHONY: fmt-check vet build check test test-cover test-race test-integration test-all sqlc-generate migrate-up migrate-down migrate-status dev-up dev-down dev-logs check-goose-dbstring sqlc mup mdown mstatus
...
sqlc-generate:
	go run $(SQLC_CMD) generate -f $(SQLC_CONFIG)
